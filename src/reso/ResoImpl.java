...
public class ResoImpl extends UnicastRemoteObject implements IReso {
    ...
    public ResoImpl() throws RemoteException {
	  ...
	  this.clients = Collections.synchronizedMap(new HashMap<String, IClient>());
	  this.scheduler = new ScheduledThreadPoolExecutor(20);
	  this.random = new Random(System.currentTimeMillis());
    }
    
    @Override
    public void declareClient(String name, IClient client) throws RemoteException {
      System.out.println("[RESO] " + name + " connected");
    ...
    public void sendMessage(String from, String to, Serializable content) throws RemoteException {
	  ...
	  scheduler.schedule(new Runnable() {
		@Override
		public void run() {
		    IClient client = clients.get(msg.to);
    ...
    public void broadcastMessage(String from, Serializable content) throws RemoteException {
      ...
      scheduler.schedule(new Runnable() {
		...
		public void run() {
			...
			Collection<IClient> clientList = clients.values();
